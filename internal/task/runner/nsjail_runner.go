...
// RunInSandbox 在NsJail沙箱中运行普通程序，返回详细的资源使用信息
...
// RunInteractiveInSandbox 在NsJail沙箱中运行交互题程序
func (nr *NsJailRunner) RunInteractiveInSandbox(runParams model.RunParams) *model.TestCaseResult {
	exePath := runParams.ExePath
	timeLimit := runParams.TimeLimit
	memoryLimit := runParams.MemLimit
	...
	// 交互题使用特殊的交互评测脚本
	scriptSrcPath := "./scripts/runner/oj_interactive_judge.sh"
	scriptDstPath := filepath.Join(exeDir, "oj_interactive_judge.sh")
	...
	if err != nil {
		return &model.TestCaseResult{
			TestCaseIndex: runParams.TestCaseIndex,
			Status:        model.StatusSE,
			Error:         fmt.Sprintf("无法读取oj_interactive_judge.sh脚本: %v", err),
		}
	}
	...
	cmd := exec.Command(
		...
		"--",
		"/bin/bash",
		"oj_interactive_judge.sh",
		filepath.Base(absExePath),          // 用户程序文件名
		"interactor.cpp",                   // 交互器文件名
		filepath.Base(runParams.InputFile), // 输入文件
		"output.txt",                       // 输出文件名
		"answer.txt",                       // 答案文件名
		"./",                               // 工作目录
	)

	// 对交互题不需要设置stdin，因为输入通过文件传递
	cmd.Stdin = nil
	...
	// 记录详细的运行信息
	zap.L().Info("NsJail interactive execution result",
		zap.Int("test_case", runParams.TestCaseIndex),
		zap.Duration("cpu_time", cpuTime),
		zap.Duration("real_time", realTime),
		zap.Int64("memory_bytes", memUsed),
